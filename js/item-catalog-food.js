(function (global) {
  ...
  function retailFoodCatalog() {
    return global.LuminousRetailFoodCatalog || safeRequire("./item-catalog-retail-food.js");
  }
  ...
  function listRetailCookieDefinitions() {
    const catalog = retailFoodCatalog();
    return catalog?.list ? catalog.list({ kind:"cookie_pack" }) : [];
  }

  function createRetailCookiePack(itemOrId, options = {}) {
    const catalog = retailFoodCatalog();
    return catalog?.createPack ? catalog.createPack(itemOrId, options) : null;
  }
