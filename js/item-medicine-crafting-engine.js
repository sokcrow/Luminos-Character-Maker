(function (global) {
  "use strict";
  if (global.LuminousMedicineCraftingEngine) {
    if (typeof module !== "undefined" && module.exports) module.exports = global.LuminousMedicineCraftingEngine;
    return;
  }
  ...
  function processedCatalog() {
    return global.LuminousMedicinalProcessedCatalog || safeRequire("./item-catalog-medicinal-processed.js");
  }
  function recipeCatalog() {
    return global.LuminousMedicineRecipeCatalog || safeRequire("./item-medicine-recipe-catalog.js");
  }
  ...
  function tagSet(input) {
    ...
    for (const key of ["tags","reagentTags","materialTags","functionalTags","craftTags","recipeRoles","requirementTags"]) {
      for (const value of input?.[key] || []) add(value);
    }
    if (out.has("venom") || out.has("toxic_secretion") || out.has("toxic_herb") || out.has("toxic_mushroom")) {
      out.add("toxin_source");
    }
    if (out.has("chemical_toxin")) out.add("toxin_source");
    if (out.has("regenerative_gel")) out.add("regenerative_reagent");
    if (out.has("blood") || out.has("hemolymph") || out.has("ichor") || out.has("organ_gland")) out.add("bio_reagent");
    return out;
  }

  function quantityOf(input) {
    const n = Number(input?.quantity ?? input?.count ?? input?.units ?? 1);
    return Number.isFinite(n) ? Math.max(0, n) : 0;
  }

  function productionValueOf(input) {
    for (const key of ["productionValueAhn","standardUnitValueAhn","unitValueAhn","valueAhn"]) {
  ...
  function validateInputs(contract, inputs=[]) {
    ...
    for (const requirement of contract?.inputRequirements || []) {
      let needed = Math.max(0, Number(requirement.quantity ?? 1) || 0);
      const allocated = [];
  ...
  function resolveCraftQuality(checkTotal, threshold) {
    ...
    if (checkTotal == null || !Number.isFinite(Number(checkTotal))) {
      return { quality:"standard", checkTotal:null, threshold, margin:null };
    }
  ...
  function craftProcessed(processId, inputs=[], options={}) {
    const catalog=processedCatalog();
    const contract=catalog?.getProcess?.(processId);
    const definition=catalog?.get?.(processId);
    ...
    const threshold=thresholdFor(contract,options);
    const craft=resolveCraftQuality(options.checkTotal,threshold);
    const values=craftValue(contract,inputs,craft.quality);
    const output={
      ...definition,
      quality:craft.quality,
      quantity:Math.max(1,Number(options.outputQuantity||1)||1),
      productionValueAhn:values.productionValueAhn,
      craft:{
        semanticCheck:contract.semanticCheck,
        requiredToolType:contract.requiredToolType,
        threshold,
        checkTotal:craft.checkTotal,
        margin:craft.margin,
        improvised:options.improvised===true,
        sourceInputs:inputCheck.allocations,
      },
    };
    return {crafted:true,type:"processed",contract,output,inputCheck,...values};
  }

  function craftForm(recipeId, inputs=[], options={}) {
    ...
    const values=craftValue(contract,inputs,craft.quality);
    const output={
      id:normalizeId(options.outputId || contract.id),
      name:String(options.outputName || contract.label),
      family:normalizeId(options.family || "medicine_crafted"),
      iconFamily:contract.outputIconFamily,
      category:"consumable",
      itemType:"consumable",
      quality:craft.quality,
    ...
    return {crafted:true,type:"finished_form",contract,output,inputCheck,...values};
  }

  function craft(id, inputs=[], options={}) {
    if (processedCatalog()?.getProcess?.(id)) return craftProcessed(id,inputs,options);
    if (recipeCatalog()?.get?.(id)) return craftForm(id,inputs,options);
    return {crafted:false,reason:"unknown_recipe_or_process"};
  }

  function validateEngine() {
    const processed=processedCatalog()?.validateCatalog?.();
    const recipes=recipeCatalog()?.validateCatalog?.();
    return {
      valid:Boolean(processed?.valid && recipes?.valid),
  ...
  const API=Object.freeze({
    VERSION,IMPROVISED_THRESHOLD_PENALTY,tagSet,quantityOf,productionValueOf,
    requirementMatches,validateInputs,thresholdFor,resolveCraftQuality,craftValue,
    craftProcessed,craftForm,craft,validateEngine,
  });
  global.LuminousMedicineCraftingEngine=API;
