(function (root, factory) {
  ...
  function modifierFromSkillValue(value) {
    if (Number.isFinite(Number(value))) return Number(value);
    if (!value || typeof value !== 'object') return null;
    for (const key of ['modifier', 'mod', 'bonus', 'total', 'value', 'valor']) {
  ...
  function modifierFromCarrier(carrier = {}) {
    ...
    for (const key of ['perceptionModifier', 'perceptionMod', 'percepcionModifier', 'percepcionMod', 'modifierPerception', 'modifierPercepcion']) {
      if (Number.isFinite(Number(carrier[key]))) return Number(carrier[key]);
    }
    const containers = [
      carrier.skills,
      carrier.habilidades,
      carrier.skillModifiers,
      carrier.skill_modifiers,
      carrier.characterBuild?.skills,
      carrier.characterBuild?.habilidades,
      carrier.build?.skills,
      carrier.build?.habilidades,
    ].filter((entry) => entry && typeof entry === 'object');
    for (const container of containers) {
      for (const [key, value] of Object.entries(container)) {
        if (!['perception', 'percepcion'].includes(normalizeKey(key))) continue;
        const modifier = modifierFromSkillValue(value);
  ...
  function perceptionModifier(token = {}, host = root) {
    for (const carrier of [token, token.actor, token.raw, token.character, token.characterBuild, token.build, host?.datosJugador]) {
  ...
  function cameraPolicyForModifier(modifier = 0) {
    const mod = finite(modifier, 0);
    let minZoom = 0.85;
    if (mod <= -2) minZoom = 0.90;
    else if (mod <= 0) minZoom = 0.85;
    else if (mod <= 2) minZoom = 0.75;
    else if (mod <= 4) minZoom = 0.65;
    else if (mod <= 6) minZoom = 0.55;
    else minZoom = 0.50;
    return Object.freeze({
      perceptionModifier: mod,
      minZoom,
      maxZoom: 5,
      maxLookFt: Math.max(40, Math.min(140, 60 + (mod * 10))),
    });
  }

  function feetToPixels(feet, mapData = {}) {
    const gridSize = Math.max(1, finite(mapData.grid?.size, 70));
    const distancePerCell = Math.max(0.001, finite(mapData.grid?.distancePerCell, 5));
    return (Math.max(0, finite(feet)) / distancePerCell) * gridSize;
  }

  function clampPointAround(point = {}, target = {}, radiusPx = 0) {
    const x = finite(point.x);
    const y = finite(point.y);
    const tx = finite(target.x);
    const ty = finite(target.y);
    const radius = Math.max(0, finite(radiusPx));
    const dx = x - tx;
    const dy = y - ty;
    const distance = Math.hypot(dx, dy);
    if (!radius || distance <= radius) return { x, y, clamped: false };
    const scale = radius / distance;
    return { x: tx + (dx * scale), y: ty + (dy * scale), clamped: true };
  }

  function tokenRulesActive({ isDm = false, mapData = {}, token = null } = {}) {
    if (!token) return false;
    if (!isDm) return true;
    return Boolean(mapData.lighting?.dmPreviewTokenId);
  }
  ...
  function isConfirmedTraversalPreview(detail = {}) {
    if (detail.traversing === true) return true;
    return detail.remote === true
  ...
  function createController({ runtime, mapData = runtime?.engine?.mapData, root: host = root } = {}) {
    ...
    function policyState() {
      ...
      if (!active) return { active: false, token, perceptionModifier: null, minZoom: 0.1, maxZoom: 5, maxLookFt: null };
      return { active: true, token, ...cameraPolicyForModifier(perceptionModifier(token, host)) };
    }

    function applyPolicy(force = false) {
      const policy = policyState();
      const key = policy.active
        ? `${clean(policy.token?.id)}|${policy.perceptionModifier}|${policy.minZoom}|${policy.maxLookFt}`
        : 'free';
      if (force || key !== lastPolicyKey) {
        lastPolicyKey = key;
        if (policy.active) {
    ...
    function queueTraversalSync(reason = 'token-traversal') {
      pendingFollowReason = reason;
      if (followFrame != null || stopped) return;
      followFrame = raf?.(() => {
        followFrame = null;
    ...
    function toggle() {
      return setEnabled(!enabled, { reason: enabled ? 'look-around' : 'follow' });
    }
    ...
    const onTokenMoved = (event) => {
      ...
      if (event?.type === 'vtt:token-preview-moved') {
        ...
        if (!isConfirmedTraversalPreview(event.detail || {})) return;
        queueTraversalSync(event?.detail?.remote ? 'remote-token-traversal' : 'token-traversal');
        return;
      }
      sync('token-moved');
    };
    ...
    const onKeyDown = (event) => {
      ...
      if ((event.code === 'KeyF' || String(event.key).toLowerCase() === 'f') && target()) {
        toggle();
        event.preventDefault?.();
      } else if ((event.code === 'Home' || event.key === 'Home') && target()) {
        recenter();
        event.preventDefault?.();
      }
    };
    ...
    function stop() {
      ...
      camera.setCenterConstraint?.(null);
      camera.setZoomBounds?.(0.1, 5);
      canvas.removeEventListener('vtt:token-preview-moved', onTokenMoved);
  ...
  return Object.freeze({
    layerOf,
    controlledToken,
    modifierFromSkillValue,
    modifierFromCarrier,
    perceptionModifier,
    cameraPolicyForModifier,
    feetToPixels,
    clampPointAround,
    tokenRulesActive,
    isConfirmedTraversalPreview,
    createController,
  });
});
