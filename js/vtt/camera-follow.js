(function (root, factory) {
  ...
  function modifierFromCarrier(carrier = {}) {
    ...
    const directKeys = [
      'perceptionModifier', 'perceptionMod', 'percepcionModifier', 'percepcionMod',
      'modifierPerception', 'modifierPercepcion',
    ];
    ...
    const containers = [
      carrier.skills, carrier.habilidades, carrier.skillModifiers, carrier.skill_modifiers,
      carrier.characterBuild?.skills, carrier.characterBuild?.habilidades,
      carrier.build?.skills, carrier.build?.habilidades,
    ].filter((entry) => entry && typeof entry === 'object');

    for (const container of containers) {
      for (const [key, value] of Object.entries(container)) {
        const normalized = normalizeKey(key);
        if (!['perception', 'percepcion'].includes(normalized)) continue;
  ...
  function clampPointAround(point = {}, target = {}, radiusPx = 0) {
    const x = finite(point.x), y = finite(point.y), tx = finite(target.x), ty = finite(target.y);
    const radius = Math.max(0, finite(radiusPx));
    const dx = x - tx, dy = y - ty, distance = Math.hypot(dx, dy);
