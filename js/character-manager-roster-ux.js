(function (root, factory) {
  ...
  function collapseKey(mode, label) {
    ...
    return `${safeMode}:${clean(label).toLocaleUpperCase()}`;
  }

  const api = Object.freeze({ normalizeTags, factionLabel, actorSearchText, collapseKey });
    ...
    try {
      ...
      state.collapsed = new Set(
        (Array.isArray(parsed.collapsed) ? parsed.collapsed : [])
          .map(String)
          // PR #544 originally stored faction labels without a namespace.
          // Preserve those preferences while moving every mode to mode:label keys.
  ...
  function decorateSections() {
    ...
    roster.querySelectorAll(".cm-roster-group").forEach((section) => {
      ...
      const factionAllowed = state.groupMode !== "faction"
        || !state.faction
        || label.toLocaleUpperCase() === state.faction.toLocaleUpperCase();
