(function (global) {
  ...
  function normalizeContestBelief(raw = {}) {
    const samples = Math.max(0, integer(raw.samples, 0));
    const wins = Math.max(0, integer(raw.wins, 0));
    const losses = Math.max(0, integer(raw.losses, 0));
    const denominator = Math.max(1, wins + losses || samples);
    const successEstimate = clamp(raw.successEstimate ?? raw.success ?? (samples > 0 ? wins / denominator : 0.5), 0, 1);
    const confidence = clamp(raw.confidence ?? (samples > 0 ? 0.20 + samples * 0.15 : 0), 0, 0.95);
    return { successEstimate, confidence, samples, wins, losses };
  }
  ...
  function createIntelState(seed = {}) {
    ...
    for (const [targetId, entry] of Object.entries(rawTargets)) {
      ...
      const contests = {};
      for (const [contestId, belief] of Object.entries(entry?.contests || {})) {
        contests[normalizeId(contestId)] = normalizeContestBelief(belief || {});
      }
  ...
  function observeDamageResult(intelRaw, observation = {}) {
    ...
    const target = intel.targets[targetId] || { observations: 0, threat: 0.5, damageTypes: {}, contests: {} };
    ...
    target.damageTypes[damageType] = {
      multiplierEstimate: ((previous.multiplierEstimate * previous.samples) + ratio) / samples,
      confidence: clamp(0.20 + samples * 0.15, 0, 0.95),
      samples,
    };
  ...
  function observeContestResult(intelRaw, observation = {}) {
    const intel = createIntelState(intelRaw || {});
    const targetId = clean(observation.targetId);
    const contestId = normalizeId(observation.contestId || observation.type || "contest") || "contest";
    if (!targetId) return intel;
    const target = intel.targets[targetId] || { observations: 0, threat: 0.5, damageTypes: {}, contests: {} };
    const previous = normalizeContestBelief(target.contests?.[contestId] || {});
    const won = observation.won === true || normalizeId(observation.result) === "win" || normalizeId(observation.result) === "success";
    const lost = observation.won === false || normalizeId(observation.result) === "loss" || normalizeId(observation.result) === "failure";
    if (!won && !lost) return intel;
    const wins = previous.wins + (won ? 1 : 0);
    const losses = previous.losses + (lost ? 1 : 0);
    const samples = previous.samples + 1;
    target.observations += 1;
    target.contests = target.contests || {};
    target.contests[contestId] = {
      successEstimate: wins / Math.max(1, wins + losses),
      confidence: clamp(0.20 + samples * 0.15, 0, 0.95),
      samples,
      wins,
      losses,
  ...
  function inferRole(raw = {}, definition = {}, sourceType = "skill") {
    ...
    if (sourceType === "universal") {
      ...
      if (id === "retreat") return "retreat";
      if (id === "grapple") return "control";
      if (id === "help") return "support";
      if (id === "improvise") return "setup";
    }
  ...
  function normalizePlanningResources(raw = {}) {
    ...
    return checks.map((entry) => {
      ...
      return {
        ...
        amount,
        available: Number.isFinite(Number(availableRaw)) ? Math.max(0, Number(availableRaw)) : null,
      };
  ...
  function normalizeSourceDescriptor(raw = {}, index = 0) {
    ...
    const defaultAdvantage = ["setup", "control", "support"].includes(role) ? 3 : 0;
    return {
      key: clean(raw.key, `${sourceType}:${normalizeId(sourceId) || index}`),
      sourceType,
      sourceId,
      definition,
      role,
      ...
      estimate: {
        ...
        advantage: finite(estimateRaw.advantage ?? raw.advantage, defaultAdvantage),
        control: finite(estimateRaw.control ?? raw.control, role === "control" ? defaultAdvantage : 0),
        support: finite(estimateRaw.support ?? raw.support, role === "support" ? defaultAdvantage : 0),
      ...
      maxUsesPerTurn: Math.max(1, integer(raw.maxUsesPerTurn ?? raw.maxPerTurn ?? definition.maxUsesPerTurn, MAX_PLANNED_SLOTS)),
      compileOptions: raw.compileOptions && typeof raw.compileOptions === "object" ? clone(raw.compileOptions) : {},
  ...
  function quickCandidateScore(candidate) {
    ...
    if (candidate.role === "control") return 12 + candidate.estimate.control + candidate.estimate.advantage * 0.5;
    if (candidate.role === "support") return 12 + candidate.estimate.support + candidate.estimate.advantage * 0.5;
    if (candidate.role === "setup") return 12 + candidate.estimate.advantage;
    if (candidate.role === "retreat") return 6 + candidate.estimate.safety;
  ...
  function candidateScore(candidate, state, context) {
    ...
    const tactical = estimate.advantage * (1 + intp.comboAwareness) + estimate.control * (0.75 + intp.intelAwareness * 0.5) + estimate.support * (0.75 + intp.comboAwareness * 0.75);
    let score = 0;

    if (goal === GOALS.KILL_TARGET) {
      score += effectiveDamage * 2.4 + tactical + estimate.safety * 0.25;
      if (candidate.role === "escape") score -= 14;
      if (candidate.role === "retreat") score -= 8;
    } else if (goal === GOALS.ESCAPE) {
      score += estimate.safety * 2.5 + estimate.recovery * 1.2 + effectiveDamage * 0.35 + estimate.control * 0.6;
      if (candidate.role === "escape") score += 42 + wisp.wisdom;
      if (candidate.role === "retreat") score += 18 + wisp.wisdom * 0.5;
    } else if (goal === GOALS.SURVIVE || goal === GOALS.PROTECT_SELF) {
      score += estimate.safety * (1.6 + wisp.dangerWeight) + estimate.recovery * 1.8 + effectiveDamage * 0.75 + estimate.control * 1.1 + estimate.support * 0.6;
      if (candidate.role === "escape") score += hpRatio <= wisp.escapeHpRatio ? 30 : 5;
      if (candidate.role === "retreat") score += hpRatio <= Math.min(0.65, wisp.escapeHpRatio + 0.15) ? 14 : 2;
    } else if (goal === GOALS.RECOVER_RESOURCE) {
      score += estimate.recovery * 2.5 + estimate.safety + effectiveDamage * 0.4 + estimate.control * 0.5;
    } else if (goal === GOALS.GAIN_ADVANTAGE) {
      score += estimate.advantage * 2.5 + estimate.control * 2.1 + estimate.support * 2.1 + effectiveDamage + estimate.safety * 0.5;
    }
    ...
    if (intp.comboAwareness > 0 && candidate.consumesTags.length) {
      score += candidate.consumesTags.filter((tag) => state.producedTags.has(tag)).length * 4 * intp.comboAwareness;
    }
    return score;
  }
  ...
  function canUseCandidateThisTurn(candidate, state) {
    const used = state.sequence.filter((entry) => entry.key === candidate.key).length;
    return used < candidate.maxUsesPerTurn;
  }
  ...
  function spendPlanningResources(candidate, state) {
    ...
    for (const resource of candidate.planningResources || []) {
      if (Number.isFinite(resource.available)) next[resource.key] = finite(next[resource.key], 0) + resource.amount;
    }
    return next;
  }
  ...
  function expandState(state, candidate, score, context) {
    ...
    const terminal = candidate.role === "escape" || candidate.role === "retreat";
    return {
  ...
  function compileCandidate(actor, candidate, options = {}) {
    const compileOptions = candidate.compileOptions && typeof candidate.compileOptions === "object" ? clone(candidate.compileOptions) : {};
    const fixedTargetId = candidate.fixedTargetId || clean(compileOptions.targetUnitId) || clean(compileOptions.targetId) || options.targetId || null;
    const common = {
      ...compileOptions,
      isAi: true,
      actionSlotId: options.actionSlotId,
      targetId: fixedTargetId,
      mainTargetId: fixedTargetId,
      metadata: {
  ...
  function planTurn(input = {}) {
    ...
    if (goal === GOALS.ESCAPE) {
      const withdrawal = candidates.filter((candidate) => candidate.role === "escape" || candidate.role === "retreat");
      if (withdrawal.length) candidates = withdrawal;
    } else {
    ...
    let beam = [{ sequence: [], score: 0, producedTags: new Set(), resourceSpent: {}, terminal: false }];
    for (let slotIndex = 0; slotIndex < slots.length; slotIndex += 1) {
      ...
      for (const state of beam) {
        ...
        for (const candidate of candidates) {
          if (!canUseCandidateThisTurn(candidate, state)) continue;
          if (!canAffordPlanningResources(candidate, state)) continue;
          next.push(expandState(state, candidate, candidateScore(candidate, state, context), context));
          expanded = true;
        }
