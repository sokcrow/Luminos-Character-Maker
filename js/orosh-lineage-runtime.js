(function (global) {
  ...
  const SIN_ALIASES = Object.freeze({
    ...
    gloom: "gloom", melancolia: "gloom", melancolía: "gloom",
  ...
  const FRAGMENTS = Object.freeze({
    wrath: Object.freeze(["strength", "fuerza", "intimidation", "intimidacion", "intimidación"]),
    envy: Object.freeze(["deception", "engano", "engaño", "stealth", "sigilo"]),
    gloom: Object.freeze(["insight", "perspicacia", "history", "historia"]),
    pride: Object.freeze(["charisma", "carisma", "persuasion", "persuasion", "persuasión"]),
    gluttony: Object.freeze(["investigation", "investigacion", "investigación", "perception", "percepcion", "percepción"]),
    lust: Object.freeze(["arcana", "arcanos", "religion", "religión"]),
  ...
  const normalizeId = (value) => String(value ?? "")
    ...
    ?.replace(/[\u0300-\u036f]/g, "")
    ?.replace(/[^a-z0-9]+/g, "_")
    ?.replace(/^_+|_+$/g, "") || String(value ?? "").trim().toLowerCase();
  ...
  function sameEntity(left, right) {
    ...
    const ids = new Set(identityValues(left));
    if (identityValues(right).some((id) => ids.has(id))) return true;
  ...
  function selectedByFallback(character = {}) {
    const build = character.characterBuild && typeof character.characterBuild === "object" ? character.characterBuild : {};
    const direct = [
      character.archetypeId, character.subclassId, character.lineageId,
      build.archetypeId, build.subclassId, build.lineageId,
    ].map(normalizeId);
    ...
    if (lists.some((list) => Array.isArray(list) && list.some((entry) => normalizeId(entry?.archetypeId || entry?.subclassId || entry?.id || entry) === ARCHETYPE_ID))) return true;

    return classEntries(character).some((entry) => {
      const classId = normalizeId(entry.classId || entry.id);
      const archetypeId = normalizeId(entry.archetypeId || entry.subclassId || entry.lineageId || entry.archetype?.id || entry.subclass?.id);
  ...
  function isSelected(character = currentCharacter() || {}) {
    ...
    if (api?.isSelected) {
      try {
        if (api.isSelected(character, ARCHETYPE_ID, CLASS_ID)) return true;
      } catch (_) {}
    }
    return selectedByFallback(character);
  }

  function getLevel(character = currentCharacter() || {}) {
    if (!isSelected(character)) return 0;
    const api = global.LuminousArchetypeEngine;
    if (api?.getClassLevel) {
      const value = Number(api.getClassLevel(character, CLASS_ID));
      if (Number.isFinite(value)) return Math.max(0, value);
    }
    const entry = classEntries(character).find((candidate) => normalizeId(candidate.classId || candidate.id) === CLASS_ID);
  ...
  function defaultState() {
    return {
      ...
      emotionalEchoes: { wrath: 0, envy: 0, gloom: 0, pride: 0, gluttony: 0, lust: 0, sloth: 0 },
      ascension: {
        available: true,
        active: false,
        roundsRemaining: 0,
  ...
  function stateKey(character = {}) {
    const id = identityValues(character)[0];
    if (id) return `id:${id}`;
  ...
  function getState(character = currentCharacter() || {}) {
    const key = stateKey(character);
    if (key) {
      if (!state.byIdentity.has(key)) state.byIdentity.set(key, defaultState());
      return state.byIdentity.get(key);
    }
    if (state.byObject && character && typeof character === "object") {
      if (!state.byObject.has(character)) state.byObject.set(character, defaultState());
      return state.byObject.get(character);
    }
    return defaultState();
  }
  ...
  function selectFragment(character = currentCharacter() || {}, sin) {
    if (!isSelected(character)) return { selected: false, reason: "orosh_not_selected" };
    const fragment = canonicalSin(sin);
    if (!Object.prototype.hasOwnProperty.call(FRAGMENTS, fragment)) return { selected: false, reason: "invalid_fragment" };
    const record = getState(character);
    record.selectedFragment = fragment;
    record.fragmentCheckBonusUsed = false;
    emit("luminous:orosh-fragment-selected", { character, fragment, state: record });
  ...
  function resetLongRest(character = currentCharacter() || {}) {
    ...
    record.ascension.slotRecoveryUsedThisTurn = false;
    emit("luminous:orosh-fragment-selection-required", { character, state: record });
    return record;
  }

  function traitsContain(traits, traitId) {
    const wanted = normalizeId(traitId);
    return (Array.isArray(traits) ? traits : []).some((trait) => normalizeId(trait?.id || trait?.name || trait) === wanted);
  }

  function checkKeys(check = {}) {
    return [
      check.abilityId, check.statId, check.ability, check.stat,
      check.skillId, check.skill, check.actionId, check.action,
  ...
  function emotionalIntent(check = {}) {
    const values = [
      check.intent, check.purpose, check.reason, check.actionType,
      ...(Array.isArray(check.tags) ? check.tags : []),
    ].map(normalizeId);
    return values.some((value) => [
      "emotion", "emotions", "emocion", "emociones", "interpret_emotions", "interpretar_emociones",
      "detect_lie", "detect_lies", "detectar_mentira", "detectar_mentiras", "lie_detection",
    ].includes(value));
  }

  function isYuanTi(entity = {}) {
    const build = entity.characterBuild && typeof entity.characterBuild === "object" ? entity.characterBuild : {};
    const race = normalizeId(entity.raceId || entity.race_id || entity.race?.id || build.raceId || build.race_id || entity.race || "");
  ...
  function applyTheatreMechanics(result, input = {}) {
    ...
    if (traitsContain(traits, TRAITS.FRAGMENTED_BLESSING)
      ...
      applied.push({ traitId: TRAITS.FRAGMENTED_BLESSING, finalPower: 2, fragment: record.selectedFragment });
    }

    const target = input.target || check.target || null;
    if (traitsContain(traits, TRAITS.EMOTIONAL_ECHO)
    ...
    try { Object.defineProperty(check, "__oroshAdjusted", { value: true, configurable: true, enumerable: false }); } catch (_) { check.__oroshAdjusted = true; }
    if (applied.length) emit("luminous:orosh-theatre-check", { character, check, applied, state: record });
    return result;
  }
  ...
  function isSpell(skill = {}, runtime = {}) {
    ...
    return type === "spell" || normalizeId(skill.sourceType || skill.source_type) === "spell" || Boolean(runtime.spell || skill.spellId || skill.spell_id);
  }

  function mindOrEmotionSpell(skill = {}, runtime = {}) {
    if (!isSpell(skill, runtime)) return false;
    if (skill.affectsMind === true || skill.affectsEmotion === true || skill.affectsEmotions === true) return true;
    const tags = [
    ...
    return tags.some((tag) => ["mind", "mental", "emotion", "emotions", "mente", "emocion", "emociones", "psychic", "psiquico", "psiquica"].includes(tag));
  }

  function canIgnoreTargetingObscurement(character = currentCharacter() || {}, condition) {
    if (!isSelected(character) || getLevel(character) < 1) return false;
  ...
  function preDispatchTrait(trait, trigger, runtime = {}) {
    ...
    if (traitId === TRAITS.PRIMORDIAL_BOND && normalizedTrigger === "before_skill" && getLevel(character) >= 30) {
      const skill = runtime.skill || runtime.spell || {};
      if (!record.primordialBondUsedThisTurn && mindOrEmotionSpell(skill, runtime)) {
        runtime.orosh = { ...(runtime.orosh || {}), primordialBondEligible: true, attackWeightBonus: 1 };
      }
    }
  }
  ...
  function installCombatEngineBridge() {
    ...
    if (originalFinalPower) {
      engine.calculateFinalPower = function (skill, headsFlipped, unit = null, ...rest) {
        ...
        if (unit && isSelected(character) && getLevel(character) >= 1 && matchingFragmentSkill(character, skill)) {
          result = numberOr(result, 0) + Math.max(1, getLevel(character) / 20);
        }
        return result;
      };
    }
    ...
    if (originalCoinDamage) {
      engine.calculateCoinDamage = function (attacker, defender, skill, ...rest) {
        ...
        if (typeof result === "number" && attacker && isSelected(character) && level >= 1 && matchingFragmentSkill(character, skill)) {
    ...
    if (originalAoE) {
      engine.calculateAoETargets = function (skill, primaryTarget, allPossibleTargets, unitAttacker, ...rest) {
        ...
        const record = getState(character);
        if (unitAttacker && isSelected(character) && getLevel(character) >= 30 && !record.primordialBondUsedThisTurn && mindOrEmotionSpell(skill, { self: unitAttacker })) {
          const baseWeight = numberOr(skill?.attackWeight ?? skill?.atkWeight ?? skill?.weight, 1);
          const augmented = { ...skill, attackWeight: baseWeight + 1, atkWeight: baseWeight + 1, weight: baseWeight + 1 };
          ...
          return originalAoE.call(this, augmented, primaryTarget, allPossibleTargets, unitAttacker, ...rest);
        }
        return originalAoE.call(this, skill, primaryTarget, allPossibleTargets, unitAttacker, ...rest);
      };
    }

    try { Object.defineProperty(engine, "__oroshLineageIntegrated", { value: true, configurable: true }); }
    catch (_) { engine.__oroshLineageIntegrated = true; }
    state.combatEngineSource = engine;
    return true;
  }
  ...
  function bindEvents() {
    ...
    global.addEventListener("luminous:rest-completed", (event) => {
      ...
      if (normalizeId(detail.type) !== "long_rest" || !detail.character || !isSelected(detail.character)) return;
