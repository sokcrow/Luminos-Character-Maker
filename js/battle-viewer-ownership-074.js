(function (global, factory) {
  ...
  function validatePlanIntegrity({ ownerPlayerId, action, combatants = {}, players = {}, allowLegacy = true } = {}) {
    const ownerId = clean(ownerPlayerId);
    ...
    return { ok: true, reason: null, unit: resolved.unit, unitId: resolved.unitId, ownerPlayerId: ownerId, ownerUid: resolved.ownerUid || null };
  }

  function authorizePlanWrite({ authUid, ownerPlayerId, action, combatants = {}, players = {}, dmMap = {}, configuredDmUid = null, allowLegacy = true } = {}) {
    const uid = clean(authUid);
    const dm = isDmUid(uid, dmMap, configuredDmUid);
    if (dm) return { ok: true, reason: null, role: "dm" };
    const integrity = validatePlanIntegrity({ ownerPlayerId, action, combatants, players, allowLegacy });
