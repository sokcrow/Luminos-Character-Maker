  ...
  // --- REPARACIÓN: LÓGICA DE ENVÍO Y LECTURA DEL TEATRO DE LA MENTE ---
  {
    ...
    if (typeof db !== "undefined") {
      ...
      const renderizarLog = (snap) => {
        ...
        // ⚡ Bolt Optimization: Replace O(N) array search inside log render loop with O(1) map lookup
        // 💡 What: Pre-compute lowercase names to actor objects in a Map outside the loop.
        // 🎯 Why: O(N^2) complexity causes lag during dynamic UI generation inside Firebase `.on('value')` listeners.
        // 📊 Impact: Turns O(n) array lookups into O(1) map lookups, speeding up theater log rendering.
