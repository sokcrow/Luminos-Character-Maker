    ...
    if (typeof db !== "undefined") {
      ...
      const renderizarLog = (snap) => {
        ...
        if (logs) {
          ...
          // 💡 What: Pre-compute actor icons map outside the loop for O(1) lookup
          const actorIcons = new Map();
          if (window.allActoresCache) {
            for (const actor of Object.values(window.allActoresCache)) {
              if (actor.nombre && actor.icono) {
                actorIcons.set(actor.nombre.toLowerCase(), actor.icono);
              }
            }
          }
          ...
          for (const [key, msg] of Object.entries(logs)) {
            ...
            if (msg.nombre) {
              ...
              if (actorIcons.has(lowerName)) {
                dynamicIcon = actorIcons.get(lowerName);
              }
            }
