...
window.recetasCache = {};
let recetasCache = window.recetasCache;
...
window.renderRecetasCrafteo = function renderRecetasCrafteo() {
    ...
    if (Object.keys(recetasCache).length === 0) {
        ...
        for (const [idReceta, receta] of Object.entries(recetasCache)) {
            ...
            btn.innerHTML = `
                <div style="width: 40px; height: 40px; background: url('${iconUrl}') center/cover; border-radius: 4px; filter: ${isDiscovered ? 'none' : 'brightness(0) drop-shadow(0 0 2px #444)'}; margin-right: 10px; position: relative;">
                    <div style="position: absolute; top: 0; left: 2px; color: #ffaa00; font-family: 'Mikodacs', sans-serif; font-size: 12px; text-shadow: 1px 1px 0 #000, -1px -1px 0 #000;">${['I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII', 'IX', 'X'][itemTier - 1] || 'I'}</div>
                </div>
                <div style="flex: 1; color: ${isDiscovered ? '#ccc' : '#666'}; font-family: 'ExcelsiorSans', sans-serif; font-size: 14px;">${isDiscovered ? itemName : 'Receta Desconocida'}</div>
            `;
            ...
            btn.onclick = () => {
                ...
                btn.style.borderColor = 'var(--cyan-tech)';
                btn.style.background = 'rgba(0, 255, 255, 0.1)';

                seleccionarRecetaRadial(idReceta, receta, isDiscovered, iconUrl, itemName, itemTier);
            };
    ...
    if (listaAlijo) {
        ...
        if (currentStash.length === 0) {
            ...
            currentStash.forEach(item => {
                ...
                const iconUrl = item.icono || globalData.icono || 'https://i.imgur.com/8QZ7XqY.png';
...
// Logic for Subtab switching in the left panel
document.addEventListener('click', (e) => {
    if (e.target.classList.contains('crafteo-subtab-btn')) {
        const subtabs = document.querySelectorAll('.crafteo-subtab-btn');
        const contents = document.querySelectorAll('.crafteo-list-content');
...
function seleccionarRecetaRadial(idReceta, receta, isDiscovered, resultIconUrl, resultItemName, resultTier) {
    ...
    if (fabricarBtn) {
        ...
        // Here we could attach the fabrication logic later or invoke the existing one
        // For now we just visually enable/disable
    }
}

function limpiarSlotsIngredientes() {
    for (let i = 1; i <= 5; i++) {
        const slot = document.getElementById(`craft-slot-${i}`);
        if (slot) {
            slot.innerHTML = '';
...
// Reemplazar la vieja lógica `seleccionarReceta` si aún está en uso o comentarla
// por la nueva `seleccionarRecetaRadial`
