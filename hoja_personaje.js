    ...
    if (typeof db !== "undefined") {
      ...
      const renderizarLog = (snap) => {
        ...
        if (logs) {
          ...
          // ⚡ Bolt Optimization: Pre-compute lowercase hash map for O(1) lookup
          const actorsByName = {};
          if (window.allActoresCache) {
            Object.values(window.allActoresCache).forEach(actor => {
              if (actor && actor.nombre) {
                actorsByName[actor.nombre.toLowerCase()] = actor;
              }
            });
          }
          ...
          for (const [key, msg] of Object.entries(logs)) {
            ...
            if (msg.nombre) {
              const actorMatch = actorsByName[msg.nombre.toLowerCase()];
