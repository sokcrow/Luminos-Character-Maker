  ...
  // --- REPARACIÓN: LÓGICA DE ENVÍO Y LECTURA DEL TEATRO DE LA MENTE ---
  {
    ...
    if (typeof db !== "undefined") {
      ...
      const renderizarLog = (snap) => {
        ...
        if (logs) {
          // Pre-compute O(1) lookup map for actors to prevent O(N²) layout rendering bottleneck
          const actorsMap = {};
          if (window.allActoresCache) {
            for (const actor of Object.values(window.allActoresCache)) {
              if (actor.nombre) {
                actorsMap[actor.nombre.toLowerCase()] = actor;
              }
            }
          }
          ...
          for (const [key, msg] of Object.entries(logs)) {
            ...
            if (msg.nombre) {
              const actorMatch = actorsMap[msg.nombre.toLowerCase()];
