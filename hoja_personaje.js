...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const handleEquipItem = async (itemKey, slotId) => {
    ...
    // Validación de tipos
    let itemTags = draggingItem.tags
      ? Array.isArray(draggingItem.tags)
        ? draggingItem.tags
        : draggingItem.tags.split(",")
      : [];
    let itemTypes = [draggingItem.tipo, ...itemTags]
      .filter(Boolean)
      .map((t) => t.trim().toLowerCase());

    let allowed = allowedTypes[slotId] || [];
    let isTypeValid =
      allowed.some((a) => itemTypes.some((t) => t.includes(a))) ||
      itemTypes.length === 0;

    if (
      slotId.includes("accesorio") &&
      (itemTypes.includes("arma") || itemTypes.includes("armadura"))
    ) {
    ...
    if (!isTypeValid) {
      alert(
        `No puedes equipar esto aquí. Slot: ${slotId}. Requiere: ${allowed.join(", ")}`,
      );
      return;
    }
    ...
    if (
      slotId.includes("arma_") &&
      itemTypes.some((t) => t.includes("fuego"))
    ) {
      console.log(
        "Arma de fuego detectada. Asegúrate de tener balas en el slot de Munición.",
      );
    }
    ...
    if (activeInvGrid) {
      ...
      activeInvGrid.addEventListener("drop", async (e) => {
        ...
        if (playerId) {
          await db
            .ref(
              `campaña/jugadores/${playerId}/inventario_activo/${itemKey}/equipped_slot`,
            )
            .remove();
        }
      });
  ...
  document.body.addEventListener(
    ...
    (e) => {
      ...
      const isEquippedItem =
        slot.classList.contains("equip-slot") && slot.dataset.equippedItemKey;

      if (isGridItem || isEquippedItem) {
        draggedItemKey =
          slot.dataset.equippedItemKey || slot.dataset.key || null;
      }
    },
    { passive: true },
  );
    ...
    if (elemBelow) {
      ...
      if (targetSlot) {
        ...
        if (playerId) {
          await db
            .ref(
              `campaña/jugadores/${playerId}/inventario_activo/${draggedItemKey}/equipped_slot`,
            )
            .remove();
        }
      }
...
db.ref("campaña/actores").on("value", (snapshot) => {
  ...
  const nombreBase =
    window.datosJugador && window.datosJugador.characterName
  ...
  if (
    window.datosJugador &&
    window.datosJugador.activeActor &&
    actorSelect.querySelector(
      `option[value="${window.datosJugador.activeActor}"]`,
    )
  ) {
    actorSelect.value = window.datosJugador.activeActor;
  } else if (
    currentSelection &&
    actorSelect.querySelector(`option[value="${currentSelection}"]`)
  ) {
    ...
    if (
      ...
      if (actor.expresiones) {
        ...
        if (Array.isArray(actor.expresiones)) {
          ...
          for (const [nombreExp, urlSprite] of Object.entries(
            actor.expresiones,
          )) {
            ...
            if (typeof urlSprite === "object" && urlSprite !== null) {
              ...
              option.textContent =
                urlSprite.nombre || urlSprite.name || nombreExp;
...
function calculateDerivedStats(data) {
  ...
  if (data.inventario_activo) {
    Object.values(data.inventario_activo).forEach((item) => {
      if (item.equipped_slot) {
        ...
        if (
          item.equipped_slot === "arma_principal" ||
          item.equipped_slot === "arma_secundaria" ||
          item.equipped_slot === "municion"
        ) {
          equipOffMod += offMod;
          ...
          item.equipped_slot === "accesorio_1" ||
          item.equipped_slot === "accesorio_2"
        ) {
          equipOffMod += offMod;
          equipDefMod += defMod;
          if (slotBonuses[item.equipped_slot]) {
  ...
  Object.keys(slotBonuses).forEach((slotId) => {
    const slotEl = document.querySelector(
      `.equip-slot[data-slot-id="${slotId}"] .slot-stat`,
    );
    if (slotEl) {
      if (slotId === "armadura") {
        ...
        slotId === "arma_principal" ||
        slotId === "arma_secundaria" ||
        slotId === "municion"
      ) {
...
window.renderCharacterSheet = function (data) {
  ...
  coreStats.forEach((stat) => {
    ...
    if (data.baseStats) {
      ...
      if (baseKey) baseVal = parseInt(data.baseStats[baseKey]) || 0;
    }
    let modVal = 0;
    if (data.modifiers) {
      ...
      if (modKey) modVal = parseInt(data.modifiers[modKey]) || 0;
    }
    ...
    const totalDisplays = document.querySelectorAll(
      `span[name="attr_${stat}"], input[name="attr_${stat}"]`,
    );
  ...
  if (data.characterName) {
    document
      .querySelectorAll(
        'input[name="attr_character_name"], span[name="attr_character_name"], div[name="attr_character_name"]',
      )
      .forEach((el) => {
        if (el.tagName === "INPUT" && document.activeElement !== el)
          el.value = data.characterName;
  ...
  if (data.ahn !== undefined) {
    document
      .querySelectorAll(
        '.sheet-banco-amount-display, #display-ahn, input[name="attr_ahn"], span[name="attr_ahn"]',
      )
      .forEach((el) => {
        if (el.tagName === "INPUT" && document.activeElement !== el)
          el.value = data.ahn;
    ...
    if (bancoBalance) {
      ...
      bancoBalance.textContent = new Intl.NumberFormat("en-US").format(
        data.ahn || 0,
      );
    }
    ...
    if (shopAhn) {
      shopAhn.textContent = new Intl.NumberFormat("en-US").format(
        data.ahn || 0,
      );
    }
  }
  ...
  if (data.combatStats) {
    ...
    const hpCoef =
      parseFloat(data.combatStats.hp_coefficient || data.hp_coefficient) || 0;
    ...
    const finalTotalOffLvl =
      dmBaseOff + derivedStats.equipOffMod + derivedStats.keywordOffBonus;
    const finalTotalDefLvl =
      dmBaseDef +
      baseDefLvlMod +
      derivedStats.equipDefMod +
      derivedStats.keywordDefBonus;
    ...
    const hpMax =
      Math.floor(hpBase + finalTotalDefLvl * hpCoef) +
        derivedStats.keywordHpBonus || 100;

    let hpActual =
      data.combatStats.hp_actual !== undefined
        ? parseInt(data.combatStats.hp_actual)
        : hpMax;
    ...
    const spActual =
      data.combatStats.sp_actual !== undefined
        ? parseInt(data.combatStats.sp_actual)
        : 0;
    ...
    if (pathHP && pathDelay && hudContainer) {
      ...
      if (
        window.lastHpActual !== undefined &&
        hpActual !== window.lastHpActual &&
        hpActual > 0 &&
        feedbackOverlay
      ) {
    ...
    if (sphere) {
      ...
      sphere.style.setProperty(
        "--sp-center",
        `rgb(${rCenter}, ${gCenter}, ${bCenter})`,
      );
      sphere.style.setProperty(
        "--sp-edge",
        `rgb(${rEdge}, ${gEdge}, ${bEdge})`,
      );
    ...
    if (portraitImg) {
      ...
      if (
        assignedActorId &&
        window.actoresJugador &&
        window.actoresJugador[assignedActorId]
      ) {
        ...
        if (dataActor) {
          ...
          currentDisplayAvatar =
            dataActor.icono || dataActor.sprite || currentDisplayAvatar;
        }
      }
    ...
    if (coinTextEl) {
      ...
      coinTextEl.style.color =
        spActual > 0 ? "#00ffff" : spActual < 0 ? "#ff4444" : "#ffffff";
    }
  }
  ...
  if (data.class) {
    document
      .querySelectorAll(`span[name="attr_class"], input[name="attr_class"]`)
      .forEach((el) => {
        if (el.tagName === "INPUT") {
  ...
  if (data.race) {
    document
      .querySelectorAll(`span[name="attr_race"], input[name="attr_race"]`)
      .forEach((el) => {
        if (el.tagName === "INPUT") {
  ...
  if (data.background) {
    document
      .querySelectorAll(
        `span[name="attr_background"], input[name="attr_background"]`,
      )
      .forEach((el) => {
        if (el.tagName === "INPUT") {
  ...
  if (data.identity) {
    document
      .querySelectorAll(
        `span[name="attr_identity"], input[name="attr_identity"]`,
      )
      .forEach((el) => {
        if (el.tagName === "INPUT") {
  ...
  if (data.identity_notes) {
    document
      .querySelectorAll(`textarea[name="attr_identity_notes"]`)
      .forEach((el) => {
  ...
  if (data.level !== undefined) {
    document
      .querySelectorAll('span[name="attr_level"]')
      .forEach((el) => (el.innerText = data.level));
  ...
  if (data.xp !== undefined) {
    document
      .querySelectorAll('span[name="attr_xp"]')
      .forEach((el) => (el.innerText = data.xp));
  ...
  if (data.xpMissing !== undefined) {
    document
      .querySelectorAll('span[name="attr_xp_missing"]')
      .forEach((el) => (el.innerText = data.xpMissing));
  }
