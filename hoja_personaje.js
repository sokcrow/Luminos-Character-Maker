...
function initializeCharacterSheet() {
  ...
  // --- REPARACIÓN: LÓGICA DE ENVÍO Y LECTURA DEL TEATRO DE LA MENTE ---
  {
    ...
    const sendTheatreMessage = () => {
      ...
      try {
        ...
        // Validate multiple actors
        const selectActor = document.getElementById("player-actor-select");
        let activeActorId = assignedActorId;
        if (selectActor && selectActor.value && window.LuminousTheatreState) {
            const assignedIds = window.LuminousTheatreState.normalizeAssignedActorIds(
                window.datosJugador.actorIds || window.datosJugador.personajes,
                window.datosJugador.actorId
            );
            if (assignedIds.includes(selectActor.value)) {
                activeActorId = selectActor.value;
            }
        }

        // Ensure values strictly from visual composer elements, as syncPlayerTheatreComposer syncs them
        const imgIcon = document.getElementById("player-theatre-icon");
        const lblName = document.getElementById("player-theatre-name");
        const lblTitle = document.getElementById("player-theatre-title");

        const payload = {
          actorId: activeActorId,
          nombre: lblName ? lblName.textContent : (actorParaEnviar.nombre || "Jugador"),
          titulo: lblTitle ? lblTitle.textContent : (actorParaEnviar.titulo || ""),
          color_nombre: actorParaEnviar.color_nombre || "#ffffff",
          color_titulo: actorParaEnviar.color_titulo || "#4a4a4a", // New default requested
          escala: isNaN(actorParaEnviar.escala) ? 1.0 : actorParaEnviar.escala,
          expression: selectedExpression,
          sprite: selectedSprite || null,
          icono: imgIcon && imgIcon.src ? imgIcon.src : finalIcon,
  ...
  window.syncPlayerTheatreComposer = function() {
    if (!window.datosJugador) return;

    const LuminousTheatreState = window.LuminousTheatreState || {
        normalizeAssignedActorIds: (v, l) => {
            let res = [];
            if(Array.isArray(v)) res = v;
            else if(v && typeof v === 'object') res = Object.keys(v).map(k=>v[k]===true?k:v[k]);
            else if(typeof v === 'string') res = [v];
            if(res.length === 0 && l && typeof l === 'string') res = [l];
            return [...new Set(res.filter(id => id && id.trim().length > 0))];
        }
    };

    const assignedIds = LuminousTheatreState.normalizeAssignedActorIds(
        window.datosJugador.actorIds || window.datosJugador.personajes,
        window.datosJugador.actorId
    );

    const btnSubmit = document.getElementById("btn-enviar-teatro");
    const selectActor = document.getElementById("player-actor-select");
    const wrapActor = document.getElementById("player-actor-select-wrap");
    const selectExpression = document.getElementById("player-expression-select");
    const infoContainer = document.getElementById("player-theatre-info");
    const warning = document.getElementById("player-theatre-warning");

    if (assignedIds.length === 0) {
        if (btnSubmit) btnSubmit.disabled = true;
        if (selectExpression) selectExpression.disabled = true;
        if (warning) warning.style.display = "block";
        if (infoContainer) infoContainer.style.display = "none";
        if (wrapActor) wrapActor.style.display = "none";
        return;
    }

    if (btnSubmit) btnSubmit.disabled = false;
    if (selectExpression) selectExpression.disabled = false;
    if (warning) warning.style.display = "none";
    if (infoContainer) infoContainer.style.display = "flex";

    let currentSelectedId = selectActor ? selectActor.value : null;
    if (!assignedIds.includes(currentSelectedId)) {
        currentSelectedId = assignedIds[0];
    }

    if (assignedIds.length === 1) {
        if (wrapActor) wrapActor.style.display = "none";
        if (selectActor) selectActor.style.display = "none";
    } else {
        if (wrapActor) wrapActor.style.display = "block";
        if (selectActor) {
            selectActor.style.display = "block";
            // Populate select only if needed
            if (selectActor.children.length !== assignedIds.length) {
                selectActor.innerHTML = '';
                assignedIds.forEach(id => {
                    const opt = document.createElement("option");
                    opt.value = id;
                    opt.textContent = "Cargando " + id + "...";
    ...
    window.firebase.database().ref("campaña/base_datos_npcs").once("value").then(npcsSnap => {
        window.firebase.database().ref("campaña/actores").once("value").then(actoresSnap => {
            const allActors = { ...(actoresSnap.val() || {}), ...(npcsSnap.val() || {}) };

            // Update actor dropdown labels
            if (selectActor && assignedIds.length > 1) {
                Array.from(selectActor.options).forEach(opt => {
                    const data = allActors[opt.value];
                    if (data) opt.textContent = data.nombre || opt.value;
                });
            }

            // Sync visual composer info for the currently selected actor
            const actorData = allActors[currentSelectedId];
            if (actorData) {
                const imgIcon = document.getElementById("player-theatre-icon");
                const lblName = document.getElementById("player-theatre-name");
                const lblTitle = document.getElementById("player-theatre-title");

                if (imgIcon) imgIcon.src = actorData.icono || 'https://via.placeholder.com/50';
                if (lblName) lblName.textContent = actorData.nombre || 'Desconocido';
                if (lblTitle) {
                    lblTitle.textContent = actorData.titulo || '';
                    lblTitle.style.display = actorData.titulo ? 'block' : 'none';
                }

                if (selectExpression) {
                    const prevExp = selectExpression.value;
                    selectExpression.innerHTML = '';
                    const exprs = actorData.expresiones || {};
                    let hasPrev = false;
                    ...
                    if (Object.keys(exprs).length === 0) {
                        ...
                        opt.dataset.sprite = actorData.sprite || '';
                        selectExpression.appendChild(opt);
                    } else {
                        Object.keys(exprs).forEach(expName => {
                            const opt = document.createElement("option");
                            opt.value = expName;
                            opt.textContent = expName;
                            opt.dataset.sprite = exprs[expName];
                            selectExpression.appendChild(opt);
                            if (expName === prevExp) hasPrev = true;
                        });
                    }

                    if (hasPrev) selectExpression.value = prevExp;
                }
            }
        });
    });
