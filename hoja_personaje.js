    ...
    if (typeof db !== "undefined") {
      ...
      const renderizarLog = (snap) => {
        ...
        if (logs) {
          ...
          // Pre-compute actor map to avoid O(N^2) lookups inside the loop
          const actorsByName = new Map();
          if (window.allActoresCache) {
            for (const actor of Object.values(window.allActoresCache)) {
              if (actor.nombre) {
                actorsByName.set(actor.nombre.toLowerCase(), actor);
              }
            }
          }
          ...
          for (const [key, msg] of Object.entries(logs)) {
            ...
            if (msg.nombre) {
              const actorMatch = actorsByName.get(msg.nombre.toLowerCase());
