...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (activeInvGrid) {
    ...
    activeInvGrid.addEventListener("drop", (e) => {
      ...
      // Unequip item
      db.ref(
        `campaña/jugadores/${playerId}/inventario_activo/${itemKey}`,
      ).update({
        equipped_slot: null,
      });
    });
...
db.ref("campaña/actores").on("value", (snapshot) => {
  ...
  if (
    ...
    actorSelect.querySelector(`option[value="${window.datosJugador.actorId}"]`)
  ) {
...
window.actualizarExpresionesDesdeDropdown = function () {
  try {
    ...
    if (
      actorAsignadoId &&
      window.actoresJugador &&
      window.actoresJugador[actorAsignadoId]
    ) {
      ...
      const optionExists = Array.from(actorSelect.options).some(
        (opt) => opt.value === actorAsignadoId,
      );
      if (optionExists) {
  ...
  if (data.combatStats) {
    ...
    if (spSphere) {
      ...
      if (spActual === -45) {
        ...
        let colorCalculado;
        if (spActual < 0) {
          colorCalculado = interpolateColor(
            neutralColor,
            maxRed,
            Math.abs(spActual) / 45,
          );
        } else if (spActual > 0) {
          colorCalculado = interpolateColor(
            neutralColor,
