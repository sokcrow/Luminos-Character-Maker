...
assert.ok(viewer.includes('let rafId=0,dirty=true,continuousUntil=0,surfaceActive=true'),'renderer must track dirty/on-demand and surface visibility state');
...
assert.ok(viewer.includes("version:'0.8.2-hybrid'"),'patched renderer must advertise hybrid demand-render mode');
...
assert.ok(viewer.includes('if(!surfaceActive&&rafId){cancelAnimationFrame(rafId);rafId=0}'),'switching away from Combat must cancel an active RAF immediately');
