...
(async () => {
  ...
  delete globalThis.LuminousFirearmRuntime;
  await import(pathToFileURL(path.join(root, 'js/item-firearm-runtime.js')).href);
  const Runtime = globalThis.LuminousFirearmRuntime;
