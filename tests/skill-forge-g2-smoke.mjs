...
for (const tier of [1, 2, 3]) {
  assert.ok(forge.TIER_RULES[tier].countCost > forge.TIER_RULES[tier].potencyCost, `Tier ${tier} Count must cost more than Potency`);
}
...
assert.ok(finalCost < coinCost, 'Final Power must be cheaper than Coin Power on multi-Coin Skills');
...
const illegalG1Condition = forge.validateRecipe({
  ...
  range: 1,
  attackWeight: 1,
  power: { basePower: 4, coinPower: 3, coinAmount: 2 },
  core: { status: 'burn', potency: 1, count: 0 },
  style: {
    id: 'predator',
    reward: { kind: 'damage_percent', amount: 10 },
    condition: { target: 'target', stat: 'hp_percent', operator: '<=', value: 50 },
  },
});
...
const tooMuchCount = forge.validateRecipe({
  ...
  range: 1,
  attackWeight: 1,
  power: { basePower: 7, coinPower: 6, coinAmount: 1 },
  core: { status: 'tremor', potency: 1, count: 2 },
...
const deckA = forge.generateDeck({ seed: 'minion-01', size: 4, tier: 1, generation: 1, weapon: 'hammer', status: 'tremor' });
const deckB = forge.generateDeck({ seed: 'minion-01', size: 4, tier: 1, generation: 1, weapon: 'hammer', status: 'tremor' });
