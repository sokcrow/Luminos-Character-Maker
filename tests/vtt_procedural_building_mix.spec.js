...
function reset(){
  ...
  for(const file of FILES)fresh(file);
}
function restore(){
  for(const file of FILES){try{delete require.cache[require.resolve(path.join(ROOT,file))];}catch(_){}}
  for(const k of KEYS){const value=ORIGINAL.get(k);if(value===undefined)delete global[k];else global[k]=value;}
}
test.beforeEach(reset);test.afterEach(restore);

function profileWithMix(profileId,buildingMix){
  const fabric=global.LuminousVttUrbanFabric;
  return{...fabric.normalizeProfile(profileId),buildingMix};
}
...
test('custom all-shop mix drives real building archetype generation and still validates',()=>{
  ...
  expect(new Set(archetypes(plan))).toEqual(new Set(['shop']));
  expect(plan.fabric.profile.buildingMix.shop).toBe(1);
});
...
test('building mix remains deterministic and is part of the generated signature',()=>{
  ...
  expect(new Set(archetypes(a))).toEqual(new Set(['shop']));expect(new Set(archetypes(c))).toEqual(new Set(['apartment_building']));
  expect(c.signature).not.toBe(a.signature);
});

test('parcel size restrictions remain authoritative over a requested building mix',()=>{
  const buildings=global.LuminousVttProceduralBuildings;
  const small={buildable:{minCol:0,minRow:0,maxCol:7,maxRow:7}};
...
test('Zone Creator exposes and forwards editable Shops/Apartments/Workshops/Warehouses mix',()=>{
  const authoring=fs.readFileSync(path.join(ROOT,'js/vtt/procedural-generator-authoring-bootstrap.js'),'utf8');
  const runtime=fs.readFileSync(path.join(ROOT,'js/vtt/procedural-generator-bootstrap.js'),'utf8');
  expect(authoring).toContain('03 · BUILDING MIX');
  for(const name of ['shop','apartment','workshop','warehouse'])expect(authoring).toContain(`data-proc-mix=\"${name}\"`);
