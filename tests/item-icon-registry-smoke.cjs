...
(async () => {
  ...
  assert.ok(registry);
  assert.equal(registry.VERSION, 13);
  assert.equal(registry.DEFAULT_GROUP, 'generic_item');
  assert.equal(Object.keys(registry.GROUPS).length, 96);
