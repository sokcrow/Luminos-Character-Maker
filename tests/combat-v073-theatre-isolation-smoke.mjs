...
assert.ok(viewer.includes('if(!surfaceActive&&rafId){cancelAnimationFrame(rafId);rafId=0}'),'hidden Combat must cancel RAF immediately');
...
for(const name of theatreFiles){
  ...
  assert.ok(!source.includes("0.8.2-hybrid"),`${name} must not depend on Combat renderer version`);
...
for(const file of ['hoja_personaje.html','hoja_de_DM.html','pantalla_dm.html']){
  ...
  assert.ok(!source.includes("0.8.2-hybrid"),`${file} must not expose Combat renderer internals globally`);
}
