...
(async () => {
  ...
  assert.equal(data.listItemIds().length, 139);
  assert.equal(new Set(data.listItemIds()).size, 139);
  ...
  console.log('Culinary affinity data smoke: OK (139 items, canonical 100-point distributions)');
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});
