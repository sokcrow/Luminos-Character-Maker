...
function installUvScroll(mat,uniform,key){mat.onBeforeCompile=shader=>{shader.uniforms.luminousUvOffset=uniform;shader.vertexShader=shader.vertexShader.replace('#include <uv_pars_vertex>','#include <uv_pars_vertex>\\nuniform vec2 luminousUvOffset;').replace('#include <map_vertex>','#include <map_vertex>\\n#ifdef USE_MAP\\n vMapUv += luminousUvOffset;\\n#endif')};mat.customProgramCacheKey=()=>key}
function waterMaterial(THREE,cfg){const uniforms={uvOffset:{value:new THREE.Vector2(0,0)}},material=new THREE.MeshStandardMaterial({color:cfg.color,map:null,roughness:cfg.roughness,metalness:cfg.metalness,transparent:cfg.opacity<.999,opacity:cfg.opacity,side:THREE.DoubleSide,depthWrite:true,depthTest:true});installUvScroll(material,uniforms.uvOffset,'luminous-water-v1');material.userData.luminousWaterUniforms=uniforms;return{material,uniforms}}
function foamMaterial(THREE,cfg){const uniforms={uvOffset:{value:new THREE.Vector2(0,0)},time:{value:0},pulseAmplitude:{value:cfg.pulseAmplitude},pulseSpeed:{value:cfg.pulseSpeed},pulseFrequency:{value:cfg.pulseFrequency}},material=new THREE.MeshBasicMaterial({color:0xffffff,map:null,transparent:true,opacity:cfg.opacity,side:THREE.DoubleSide,depthWrite:false,depthTest:true});
...
function disposeObject(o){o?.traverse?.(c=>{c.geometry?.dispose?.();const ms=Array.isArray(c.material)?c.material:[c.material];for(const m of ms)m?.dispose?.()});o?.parent?.remove?.(o)}
export function createWorldWaterBodyRuntime({THREE,renderer=null,baseUrl=import.meta.url,logger=console,showShoreFoamRibbon=false}={}){
  ...
  const attach=(rec,kind,path,mat)=>textures.load(path,kind).then(tex=>{if(tex&&!rec.disposed){mat.map=tex;mat.needsUpdate=true}return tex});
