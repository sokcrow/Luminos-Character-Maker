...
assert.match(game, /TerrainMobility\.js/);
assert.match(game, /TerrainSurfaceStandard\.js/);
...
assert.match(game, /dir:'start'/);
assert.match(game, /SAFE_SPAWN_ROUTE_LIMIT=24/);
...
assert.match(game, /__paperLoadLastProgressAt/);
assert.match(game, /idleMs>30000/);
...
assert.match(game, /function riverCalmShoulderWidthAtT\(river,t\)/);
assert.match(game, /function riverWaterHalfWidthAtT\(river,t\)/);
...
assert.match(game, /source:includeCalmShoulder\?'riverFieldSample\+riverWaterHalfWidthAtT':'riverFieldSample\+riverHalfWidthAtT'/);
...
assert.match(game, /surfaceWave:\{/);
assert.match(game, /secondaryFrequency:6\.6/);
assert.match(game, /edgeStrength:\.22/);
assert.match(game, /River chop is visual-only/);
assert.match(game, /opacity:1/);
assert.match(game, /surface:\{renderOrder:-4,depthWrite:true\}/);
assert.match(game, /riverTerminalCaps:!!terminalCaps/);
assert.match(game, /width:TILE\*\.48/);
assert.doesNotMatch(game, /id:\`river-calm:/);
assert.doesNotMatch(game, /id:\`river-current:/);
...
assert.match(game, /lightingInfluence:0/);
assert.match(game, /paperFXWaterLightingInfluence/);
assert.match(game, /outgoingLight = mix\(diffuseColor\.rgb, outgoingLight, paperFXWaterLightingInfluence\)/);
assert.match(game, /surface:\{enabled:false\}/);
assert.match(game, /tileWorldLength:TILE\*3\.10/);
assert.match(game, /maskScale:2\.60,maskWaveAmplitude:\.14/);
assert.match(game, /pulseAmplitude:TILE\*\.060/);
assert.match(game, /detail:\{[\s\S]*scrollSpeed:-\.012/);
assert.match(game, /shorelineDryLiftTiles:\.14/);
assert.match(game, /shorelineRaiseWidthTiles:\.58/);
assert.match(game, /clipLandMargin:\.12/);
assert.match(game, /createWaterInteractionSystem/);
assert.match(game, /window\.WaterInteractionRuntime/);
assert.match(game, /showWakeDebug/);
assert.match(game, /registerStaticWaterObstacle/);
assert.match(game, /type:'staticObstacle'/);
assert.match(game, /waterInteractionSystem\.registerInteractor\(\`unit:\$\{id\}\`/);
assert.match(game, /flowAt:\(p\)=>\{/);
assert.match(game, /surfaceHeightAt:\(\)=>baseY/);
assert.match(game, /waterInteractionSystem\.clearNonPersistent\(\)/);
assert.match(game, /waterInteractionSystem\.update\(dt\)/);
assert.match(game, /Assets\/Images\/World\/Water\/wake_trail\.png/);
assert.match(game, /function continuousHydrologyBankAtWorld\(x,z\)/);
assert.match(game, /const proceduralHydro=isOverworldSector\(mapId\)&&activeProceduralZone\?\.hydrology\?\.type!=='none'/);
assert.match(game, /continuousHydrologyBankAtWorld\(fromX,fromZ\)/);
assert.match(game, /continuousHydrologyBankAtWorld\(toX,toZ\)/);
assert.match(game, /worldFloorRepeatForTiles/);
assert.match(game, /if\(worldTextureId\)\{/);
assert.match(game, /registerMaterial\(m,worldTextureId,\{repeat:\[ru,rv\]\}\)/);
assert.match(game, /terrainGradientGroundMaterial\(\{biomeProfile,baseKind,uTiles,vTiles\}\)/);
assert.match(game, /terrainSurfaceStandard='height-gradient-v1'/);
assert.match(game, /const baseKind=module\.topKind\|\|'groveStone'/);
assert.match(game, /applySlopeSurfaceGroups\(geo,verts,triangles,\{id:mapId,baseKind,biomeProfile,uTiles,vTiles\}\)/);
assert.match(game, /fittedPolygonMaterial\(mat\(kind\),points\)/);
...
assert.match(game, /player-free-camera-v3/);
assert.match(game, /mouseRadiansPerPixel:\.0024/);
assert.match(game, /touchRadiansPerPixel:\.0062/);
assert.match(game, /function rotatePlayerCameraBy/);
assert.match(game, /function togglePlayerCameraMouseCapture/);
assert.match(game, /e\.code==='ShiftLeft'/);
assert.match(game, /requestPointerLock/);
assert.match(game, /document\.pointerLockElement===renderer\.domElement/);
assert.match(game, /localPitch:true/);
assert.match(game, /hexYawOnly:true/);
assert.match(game, /if\(!globalMapActive\)setPlayerCameraPitch/);
assert.match(game, /Global hex map: fixed elevation, horizontal orbit only/);
assert.match(game, /keyboardCameraMovement:false/);
assert.match(game, /playerCameraPitchOffset/);
assert.match(game, /orbitPitch=THREE\.MathUtils\.clamp/);
...
assert.doesNotMatch(game, /keyboard:Object\.freeze\(\{left:'Q',right:'E'\}\)/);
...
assert.match(game, /const selfPropelled=Math\.hypot\(Number\(s\.movement\.desiredX\)\|\|0,Number\(s\.movement\.desiredZ\)\|\|0\)>\.02/);
assert.match(game, /let next=s\.vertical\.falling\?'fall':\(visualMoved>\.0001\?'walk':\(talking\?'talk':'idle'\)\)/);
assert.doesNotMatch(game, /s\.locomotion==='swim'\?'swim'/);
assert.match(game, /visibleTopFraction:SWIM_VISIBLE_TOP_FRACTION,idleRipples:false/);
